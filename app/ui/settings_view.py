...
class PluginRow(QWidget):
    """单个插件条目：名称/版本、作者/描述、状态、启停开关、重载/删除。"""
    ...
    def __init__(self, rec, parent=None):
        ...
        col = QVBoxLayout()
        col.setSpacing(2)
        from app.services.plugins import _i18n_text
        name = _i18n_text(rec.plugin.name) if rec.plugin else rec.pid
        ver = rec.plugin.version if rec.plugin else ""
        title = StrongBodyLabel(f"{name}  {ver}".strip(), self)
        col.addWidget(title)
        desc_parts = []
        if rec.plugin and rec.plugin.author:
            desc_parts.append(str(rec.plugin.author))
        desc = _i18n_text(rec.plugin.description) if rec.plugin else ""
        if desc:
            desc_parts.append(desc)
        dlab = CaptionLabel(" · ".join(desc_parts), self)
        ...
        slab = CaptionLabel(L(f"状态：{state_text}", f"State: {state_text}"), self)
        if rec.state == "error":
            slab.setStyleSheet("color:#e81123;")
            if rec.error:
                slab.setToolTip(rec.error)
        col.addWidget(slab)
        if rec.state == "error" and rec.error:
            err = CaptionLabel(str(rec.error).splitlines()[0], self)
            err.setStyleSheet("color:#e81123;")
        ...
        sw.setChecked(rec.state == "loaded")
        sw.blockSignals(False)
        sw.checkedChanged.connect(lambda v, pid=rec.pid: plugin_manager.set_enabled(pid, v))
        btn_row1 = QHBoxLayout()
        btn_row1.addWidget(sw)
        ...
        del_btn = PushButton(L("删除", "Remove"), self)
        del_btn.clicked.connect(lambda _=False, pid=rec.pid, name=name: self._remove(pid, name))
    ...
    def _remove(self, pid, name):
        ...
        box = MessageBox(L("删除插件", "Remove Plugin"),
                         L(f"确定删除插件“{name}”？插件文件将从磁盘移除。",
                           f"Remove plugin \"{name}\"? Its files will be deleted."), self.window())
        if not box.exec():
            return
...
class SettingsView(ScrollArea):
    def __init__(self, parent=None):
        ...
        # 插件
        plugcard = SimpleCardWidget(self.view)
        pl = QVBoxLayout(plugcard)
        pl.setContentsMargins(20, 16, 20, 16)
        pl.setSpacing(6)
        pl.addWidget(StrongBodyLabel(L("插件", "Plugins"), plugcard))
        warn = CaptionLabel(L(
            "插件为第三方代码，拥有与主程序相同的权限，请仅安装可信来源的插件；"
            "插件行为同样受免责声明约束。",
            "Plugins are third-party code with the same privileges as the app. "
            "Install only from trusted sources; the disclaimer applies to plugin behavior."), plugcard)
        warn.setWordWrap(True)
        pl.addWidget(warn)
        self.pluginListLayout = QVBoxLayout()
        ...
        brow.addStretch(1)
        pl.addLayout(brow)
        root.addWidget(plugcard)
        from app.services.plugins import plugin_manager
        plugin_manager.changed.connect(lambda: QTimer.singleShot(0, self._refresh_plugins))
    ...
    def _refresh_plugins(self):
        ...
        if not records:
            empty = CaptionLabel(L("暂无插件。点击“导入插件…”添加 .py 插件文件，"
                                   "或将插件放入插件目录后重新扫描。",
                                   "No plugins yet. Use \"Import Plugin…\" to add a .py file, "
                                   "or drop plugins into the folder and rescan."), self.view)
            empty.setWordWrap(True)
            self.pluginListLayout.addWidget(empty)
            return
        for rec in records:
            self.pluginListLayout.addWidget(PluginRow(rec, self.view))
    ...
    def _import_plugin(self):
        ...
        if ok:
            InfoBar.success(L("导入成功", "Imported"), msg, parent=self.window())
        else:
    ...
    def _rescan_plugins(self):
        ...
        self._refresh_plugins()
        InfoBar.success(L("扫描完成", "Scan complete"),
                        L(f"共加载 {n} 个插件", f"{n} plugin(s) loaded"), parent=self.window())
